...
vector<vector<int>> shortestPath(Graph graph, vector<int> sources, vector<int> destinations) {
  ...
  // Remove the current vertex from the paths vector for each destination.
  for (int destination : destinations) {
    paths[destination].pop_back();
  }
...
int main() {
  ...
  // Create the set of destination vertices.
  vector<int> destinations = {2, 3};

  // Find the shortest paths between the source vertices and the destination vertices.
  vector<vector<int>> paths = shortestPath(graph, sources, destinations);
